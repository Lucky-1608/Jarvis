...
import { Cpu, Database, Network, Server, CheckCircle2, CircleDashed, Clock, ChevronRight, Search, FileText, Code2, Smartphone, WifiOff, Wifi } from 'lucide-react';
...
export function RightPanel() {
  const { currentTask, messages, systemStats, whatsappStatus, whatsappQr, backendConnected } = useJarvisStore();
  ...
  return (
    <motion.div 
      ...
      className="w-80 h-full border-l border-[var(--border-subtle)] bg-[var(--bg-panel)] backdrop-blur-xl flex flex-col z-10 shrink-0 overflow-y-auto"
    >
      <div className="p-5 flex flex-col gap-8">
        ...
        {/* Terminal Link */}
        <section>
          <h3 className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] font-semibold mb-3">Terminal Link</h3>
          <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
            {messages.map(msg => (
              <div key={msg.id} className={`flex flex-col gap-1 text-xs py-1 ${msg.role === 'user' ? 'text-blue-400' : 'text-zinc-300'}`}>
                <div className="flex items-center gap-2">
                  <span className="font-semibold uppercase text-[9px]">{msg.role}</span>
                  <span className="text-[10px] opacity-70 flex items-center gap-1"><Clock size={10} />{msg.timestamp}</span>
                </div>
                <div className="pl-1 border-l border-zinc-700/50 break-words">{msg.text}</div>
              </div>
            ))}
          </div>
        </section>
